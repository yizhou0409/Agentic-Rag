...
def load_docs(corpus, doc_idxs: List[int]):
    doc_idxs = [int(idx) for idx in doc_idxs]
    
    # if isinstance(corpus, datasets.Dataset):
    #     results = corpus.select(doc_idxs, keep_in_memory=True)
    # elif isinstance(corpus, list):
    #     results = [corpus[idx] for idx in doc_idxs]
    # else:
    #     raise ValueError("Unsupported corpus type. Must be a Hugging Face dataset or a list of dictionaries.")

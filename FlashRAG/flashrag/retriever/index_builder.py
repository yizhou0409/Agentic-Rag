...
class Index_Builder:
    ...
    def __init__(
        ...
        if self.num_shards > 1:
            self.corpus = self.corpus.shard(num_shards=self.num_shards, index=self.shard_id)
            print(f"Shard {self.shard_id+1} of {self.num_shards}.")
            self.embedding_save_path = os.path.join(self.save_dir, f"emb_{self.retrieval_method}_{self.shard_id+1}of{self.num_shards}.memmap")
        else:
    ...
    def build_bm25_index_bm25s(self):
        ...
        corpus = self.corpus
        is_zh = judge_zh(corpus[0].get('contents', corpus[0].get('text', '')))
    ...
    def encode_all(self):
        ...
        print(f"Start encoding {len(encode_data)} documents...")
        if self.gpu_num > 1:
            print("Use multi gpu!")
    ...
    def load_encoder(self):
        """
        Load the encoder model and return the hidden size of the model.
        """
        if self.is_clip:

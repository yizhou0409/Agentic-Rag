...
if __name__ == "__main__":
    ...
    def process_document(item):
        title, text = item
        chunks = chunker.chunk(text)
        return [{"title": title, "text": chunk.text} for chunk in chunks]

    documents_items = list(zip(all_title, all_text))
    chunksize = max(1, len(documents_items) // (args.num_workers * 4))
    clean_corpus = process_map(process_document, documents_items, max_workers=args.num_workers, chunksize=chunksize)
    clean_corpus = [item for sublist in clean_corpus for item in sublist]

...
class AlpacaDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        # --- Dynamic prompt template wrapping ---
        user_content = "\n".join(query)
        prompt_template = getattr(self.data_args, "prompt_template", None)
        if prompt_template:
            user_content = prompt_template.replace("{question}", user_content)
        prompt.append({"role": Role.USER.value, "content": user_content})
        # --- End dynamic wrapping ---

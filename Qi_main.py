...
class InferenceConfig:
    ...
    e5_model_path: str = "/gpfsnyu/scratch/qs2196/.cache/models/e5-large-v2"
    # Generation settings
    max_turns: int = 5
...
class Reasoner:
    ...
    def _load_prompt_template(self):
        ...
        prompt_path = "prompts/new_default_QA.yaml"
...
class InferenceSystem:
    ...
    def parse_response(self, response: str, question_id: str, turn_num: int) -> Tuple[Optional[str], Optional[str]]:
        """
        Parse response with priority logic: search first, then answer.
        This prevents the model from stopping too early when there's still a search query.
        
        Args:
            response: The generated response text
            question_id: Question ID for logging purposes
            turn_num: Current turn number for logging purposes
            
        Returns:
            Tuple of (search_query, answer) where only one should be non-None
        """
        # First, check for search query
        search_query = self._extract_search_query(response)
        
        if search_query:
            # If search query exists, check if there's also an answer (which should be ignored)
            answer = self._extract_answer(response)
            if answer:
                logger.warning(f"Question {question_id}, Turn {turn_num}: Early <answer> detected but ignored due to presence of <search>. Answer was: '{answer[:100]}...'") 
            return search_query, None
        
        # Only if no search query, then check for answer
        answer = self._extract_answer(response)
        if answer:
            return None, answer
        
        # Neither search nor answer found
        return None, None
    ...
    def process_dataset(self, dataset_path: str) -> List[Dict[str, Any]]:
        ...
        for turn_num in range(self.config.max_turns + 1):
            ...
            for question_data in tqdm(active_questions, desc="Processing questions"):
                ...
                # Parse response with priority logic (search first, then answer)
                search_query, answer = self.parse_response(response, question_data["id"], turn_num + 1)
        ...
        for result in all_results:
            if result["answer"]:
                ...
                # Add cover_match metric if not present (for consistency)
                if "cover_match" not in metrics:
                    metrics["cover_match"] = 0.0
...
def main():
    ...
    if args.dataset == "hotpotqa":
        dataset_path = "datasets/hotpotqa/dev.jsonl"
    else:  # 2wikimultihop
        dataset_path = "datasets/2wikimultihop/queries.jsonl"
